<template>
  <div class="login_box">
    ...
    <div class="login_form">
      <div>
        <el-form :rules="loginFormRules" :model="form" ref="loginFormRef">
          ...
          <el-form-item class="" prop="password">
            <el-input clearable show-password placeholder="请输入真是姓名" v-model="form.password">
              <template v-slot:prefix>
                <i class="el-input__icon el-icon-lock"></i>
              </template>
            </el-input>
          ...
          <el-form-item class="no_margin" prop="password">
            <el-input clearable show-password placeholder="请输入重复密码" v-model="form.password">
              <template v-slot:prefix>
                <i class="el-input__icon el-icon-lock"></i>
              </template>
            </el-input>
      ...
      <template>
        <div class="check">
          <el-link>回到登录</el-link>
        </div>
      </template>
      <el-button class="btn_login" >注册</el-button>
  ...
  methods: {
    async login () {
      this.$refs.loginFormRef.validate(async (valid) => {
        ...
        // 请求登录接口
        if (valid === true) {
          //   login({ username: this.form.username, password: this.from.password }).then((res) => {
          ...
          try {
            ...
            // localStorage.setItem('mall', data.token)
            set('zmall', data.token)
          } catch (err) {
            console.log(err)
          }
        }
